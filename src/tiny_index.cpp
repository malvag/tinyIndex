...
char *tiny_index::get(char *key) {
    ...
    if (found == lookup_.end()) return NULL;
    ...
    uint32_t key_size;
    uint32_t value_size;
    memcpy(&key_size, blob_buffer + found->second.blob_offset,
           sizeof(key_size));
    memcpy(&value_size,
    ...
    if (tb_handle->table[LOOKUP_TABLE_BLOB_ID] == NULL) {
        ...
        while (current_block || first_block) {
            ...
            for (int i = 0; i < lindex->map_size; i++) {
                ...
                if (DEBUG)
                    printf(
                        "Recovered index for kv in blob %lu by offset %lu | "
                        "size: %lu\n",
            ...
            for (; i < lindex->map_size; i++) {
                ...
                if (tb_read_blob(lnode.blob_id, buffer) < 0) {
                    printf(
                        "ERROR: recover_unordered_map: Could not read blob\n");
                    // handle error
                    exit(EXIT_FAILURE);
                }
                if (DEBUG)
                    printf(
                        "Attempting to recover kv_pair with offsey %lu in blob "
                        "%u\n",
                ...
                */
                lookup_.emplace(std::string(kv_pair->key, kv_pair->key_size),
                                lnode);
                free(kv_pair->key);
                //      free(kv_pair->value);
                free(kv_pair);
